@php
    ...
    // Login is role-agnostic — the post-login redirect routes each user to the
    // right portal. The role-themed entry routes still pass $loginRole; we use
    // it only to point the "create an account" link at the matching signup.
    $loginRole = $loginRole ?? 'client';
    $registerHref = match ($loginRole) {
        'professional' => route('register', ['role' => 'professional']),
...
<head>
    ...
    <style>
        *,*::before,*::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .rg-left { position:relative; overflow:hidden; padding:44px 52px; background:linear-gradient(155deg,#fdf1ea 0%,#fdeef2 55%,#f6effb 100%); display:flex; flex-direction:column; }
        ...
        /* ── Right form side ── */
        .rg-right { position:relative; display:flex; flex-direction:column; justify-content:center; padding:26px 60px 44px; overflow:hidden; }
        .rg-right::before { content:''; position:absolute; inset:0; background:linear-gradient(180deg,#f7f9fd 0%,#fff 22%); z-index:0; }
        .rg-topbar { position:absolute; top:26px; right:60px; z-index:2; font-size:14px; color:var(--muted); }
        .rg-topbar a { color:var(--brand); font-weight:700; margin-left:6px; }
        .rg-topbar a:hover { text-decoration:underline; }

        .rg-card { position:relative; z-index:2; width:100%; max-width:440px; margin:0 auto; }
        ...
        .rg-input { width:100%; padding:14px 15px 14px 44px; border:1.5px solid var(--line); border-radius:12px; font-family:var(--ff-body); font-size:14.5px; color:var(--ink); background:#fff; transition:border-color .15s, box-shadow .15s; }
        .rg-input::placeholder { color:#9aa6b8; }
        .rg-input:focus { outline:none; border-color:var(--brand); box-shadow:0 0 0 3px color-mix(in srgb, var(--brand) 15%, transparent); }
        .rg-input.is-invalid { border-color:#ef4444; }
        ...
        .rg-row { display:flex; align-items:center; justify-content:space-between; margin:4px 0 20px; }
        .rg-remember { display:flex; align-items:center; gap:9px; font-size:14px; color:var(--text); }
        .rg-remember input { width:16px; height:16px; accent-color:var(--brand); }
        .rg-forgot { font-size:13.5px; color:var(--brand); font-weight:600; }
        ...
        .rg-alert { padding:12px 15px; border-radius:11px; font-size:13.5px; margin-bottom:18px; }
        .rg-alert.err { background:#fef2f2; border:1px solid #fecaca; color:#b91c1c; }
        .rg-alert.ok { background:#ecfdf5; border:1px solid #a7f3d0; color:#047857; }
        .rg-alert div { margin:1px 0; }

        @media (max-width: 1080px) { .rg-right { padding:26px 34px 40px; } .rg-left { padding:40px 38px; } .rg-topbar { right:34px; } }
        @media (max-width: 900px) {
            .rg-page { grid-template-columns:1fr; }
            .rg-left { display:none; }
            .rg-right { padding:66px 22px 40px; }
            .rg-oauth { grid-template-columns:1fr; }
            .rg-topbar { right:22px; top:22px; }
        }
    </style>
</head>
<body>
<div class="rg-page">
    {{-- ── Left marketing panel (identical to signup) ── --}}
    <aside class="rg-left">
        ...
        <div class="rg-hero">
            ...
            <div class="rg-feat">
                ...
                <div class="rg-ft"><b>Pick Up Where You Left Off</b><span>Jump straight back into your events, bookings and messages.</span></div>
            </div>
            <div class="rg-feat">
    ...
    {{-- ── Right sign-in form ── --}}
    <main class="rg-right">
        <div class="rg-topbar">New here? <a href="{{ $registerHref }}">Create an account</a></div>

        <div class="rg-card">
            <h2 class="rg-h2">Welcome Back</h2>
            <div class="rg-h2-sub">Sign in to your account to continue.</div>
            ...
            @if (session('status'))
                <div class="rg-alert ok">{{ session('status') }}</div>
            @endif
            @if ($errors->any())
                <div class="rg-alert err">@foreach ($errors->all() as $error)<div>{{ $error }}</div>@endforeach</div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" id="rgLoginForm">
                ...
                <div class="rg-field">
                    ...
                    <div class="rg-wrap">
                        ...
                        <input type="email" name="email" class="rg-input {{ $errors->has('email') ? 'is-invalid' : '' }}" value="{{ old('email') }}" placeholder="Enter your email" required autofocus>
                    </div>
                </div>
                ...
                <div class="rg-field">
                    ...
                    <div class="rg-wrap">
                        ...
                        <input type="password" name="password" id="rgLoginPw" class="rg-input" placeholder="Enter your password" required>
                        <button type="button" class="rg-eye" data-eye="rgLoginPw" aria-label="Show or hide password"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
                    </div>
                </div>

                <div class="rg-row">
                    <label class="rg-remember"><input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}> Remember me</label>
                ...
                @if($showRecaptcha && $recaptchaSiteKey)
                    ...
                    @else
                        <input type="hidden" name="g-recaptcha-response" id="rgLoginRecaptcha">
                        @error('g-recaptcha-response') <div class="rg-err">{{ $message }}</div> @enderror
                    @endif
                @endif

                <button type="submit" class="rg-submit" id="rgLoginSubmit">Sign In
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                </button>
            </form>

            <div class="rg-foot">Don't have an account? <a href="{{ $registerHref }}">Sign up for free</a></div>
        </div>
    </main>
</div>
...
<script>
    ...
    document.getElementById('rgLoginSubmit').addEventListener('click', function (e) {
        e.preventDefault();
        grecaptcha.ready(function () {
            grecaptcha.execute('{{ $recaptchaSiteKey }}', {action: 'login'}).then(function (token) {
                document.getElementById('rgLoginRecaptcha').value = token;
                document.getElementById('rgLoginForm').submit();
            });
        });
    });
    @endif
