    ...
    <div class="pay-card" style="padding:0;overflow:hidden;">
        ...
        <div style="overflow-x:auto;">
            <table class="pay-table">
                <thead><tr><th style="padding-left:18px;">Professional</th><th>Status</th><th>Amount</th><th>Date</th><th style="padding-right:18px;"></th></tr></thead>
                <tbody>
                        ...
                        <tr>
                            <td style="padding-left:18px;">
                                <div class="pay-vendor">
                                    <img src="{{ $t->supplier?->avatar_url ?? \App\Models\User::placeholderAvatarUri() }}" loading="lazy" alt="">
                                    <div><div class="nm">{{ $t->supplier?->name ?? 'Professional' }}</div><div class="sub">{{ \Illuminate\Support\Str::limit($t->supplier?->profile?->headline ?? $t->event?->title ?? '', 18) }}</div></div>
                                </div>
                            </td>
    ...
    <div class="pay-rail-card">
        ...
        @forelse($transactions->take(4) as $t)
            <div class="pay-act-row">
                ...
                <div class="pay-act-body"><div class="pay-act-name">{{ ucfirst($t->status) }} · {{ \Illuminate\Support\Str::limit($t->supplier?->name ?? 'Professional', 18) }}</div><div class="pay-act-time">{{ $t->updated_at?->humanAgo() }}</div></div>
            </div>
        @empty
