...
@php
    $lvlColor = ['beginner'=>'#16a34a','intermediate'=>'#2563eb','advanced'=>'#15803d'];
    $catIcons = [
        ...
        'Marketing & Promotion' => ['#16a34a','#ecfdf3','<path d="M3 11l16-5v12L3 13z"/>'],
        'Analytics & Insights'  => ['#7c3aed','#ede9fe','<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>'],
        'Earnings & Payouts'    => ['#15803d','#dcfce7','<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>'],
...
<div class="rs-cats" style="margin-bottom:24px;">
    @foreach($categories as $cat => $count)
        @php [$c,$bg,$glyph] = $catIcons[$cat] ?? ['#16a34a','#ecfdf3','<circle cx="12" cy="12" r="9"/>']; @endphp        <div class="rs-cat"><div class="ic" style="background:{{ $bg }};color:{{ $c }};"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">{!! $glyph !!}</svg></div><b>{{ $cat }}</b><span>{{ $count }} {{ Str::plural('Course',$count) }}</span></div>
    @endforeach
</div>
...
<div class="rs-courses">
    @foreach($courses as $crs)
        @php $lc = $lvlColor[$crs->level] ?? '#16a34a'; @endphp        <div class="rs-course">
