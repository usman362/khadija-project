...
| Item | Notes |
|---|---|
| **26 Aug walkthrough — the reported bugs** | 2026-08-27. **Compare** was not broken: the packages ticked were in another state, the same-state rule dropped them, and the page said "Compare 1 Package" with no account of the rest — it now names how many went and why. **Proposals rail** — the Owner spotted two duplicate buttons; all four were the same defect ("Send Reminder", "Schedule Call", "Follow Up" all opened the inbox, "Share Availability" opened search). Cut to the two things they actually do. **Message attachments** — two bugs: the tile in the thread was a `<div>`, so a file could be seen and never opened, and a freshly sent file was dropped from the live bubble until reload. Fixed in both inboxes; `url`/`size_label` are appended on the model so any JSON carries them. **Find Professionals** now lives at `/find-professionals` (`/browse` redirects; the route NAME is unchanged so 60+ views are untouched). **Smart checklist** — every generated task is editable and removable; the hand-built list always had both. **Report CSV** — rows of four different widths in one sheet, so nothing aligned; now one square Section/Item/Value table with a UTF-8 BOM so Excel stops mangling it. **Cancellations button** — "Report something" said nothing to either side; it is role-aware now. **Room-size validator** — checked, no bug: valid input returns a full result and every invalid case returns a clean 422. It was the meeting's network. |
| **Spending page — invented money left behind** | 2026-08-27, found while resizing a column the Owner said "looks blank". The column was the smallest thing wrong with it. Three gateway cards were still live — the same three the Payments page shed on 2026-08-15: **Stripe Outflow** (a provider we are not connected to, over the paid total relabelled), **Secure Payment Vault** (another gateway we do not use; its "$0.00 currently in dispute" was typed), and **1099 Compliance Hub**, whose "W-9 Forms Collected" was `$vendors->total() - 1` — **an IRS figure produced by subtracting one**, shown to a client who might act on it at tax time. With them, a hardcoded "Critical Alert: a vendor is nearing the $600 IRS 1099 threshold" and a Send W-9 Reminder button with no action behind it. All removed. Also "Revenue Pipeline · This Month" on a client's page — a client has no revenue and the figures were all-time; now "Where your money is · All time". |
| **Registration disclosure — location & state** | 2026-08-26, built to Sir Peter's document (`GigResource_Registration_Disclosure_Location_State_2026-08-26.docx`). Last block before the Create Account button: what location we use, the same-state limit with his Maryland/Pennsylvania example, and that the limit is temporary. **Three separate checkboxes, all required, no bypass** — separate because they are three different things to understand, and one combined tick would let somebody agree to a legal limit on who they may hire without reading it. The button stays disabled until all three are ticked, and the **validator is the real gate** (verified: a POST without them is refused and no account is created). Acceptance is recorded in `registration_disclosures` — user, timestamp in **UTC**, `version = location_state_v1`, IP — versioned because the limit is temporary and a row must say which wording that person actually saw. Applies to Clients and Professionals; **Influencers are excluded** (never matched by state), and switching to Influencer clears the boxes so a tick cannot survive unread. |
| **Which location a request is matched on — ANSWERED by Sir Peter** | 2026-08-25. His question was what to use when the client picks no venue; his answer: **the client's address**. So the rule is: match on the **event location** when one is given, and fall back to the **client's own address** when it is not. **This is already what the code does** — `StateMatching::requestState()` takes the event state if the client chose one and returns `stateOf($client)` otherwise, and it is what BR, ER and the Virtual Hub all call. The BR form asks "State the event is in", defaults it to the client's own state, and says plainly "Professionals in this state are the ones who can bid", so the client can see and change it. Nothing to build. |
| **⚠️ The State Boundary Rule REVERSES a locked rule — R38's closing amendment / R71** | 2026-08-26. R71 said a request carries the state of the **WORK**, not the client's account, and the review that locked it gave a concrete reason: *a client registered in Virginia may hold an event at their office in Maryland*, and filling the request from the client's account would send it to people who cannot work it. Sir Peter's State Boundary Rule says the opposite, deliberately and in writing, because "each state has its own rules/laws". **So R71's Virginia case is now the accepted cost, not a bug**: that client reaches Virginia professionals only, and nobody in Maryland. `RequestEventStateTest` used to assert R71 and has been rewritten to the new rule, with the old expectation kept beside each test — the limit is temporary, and that file is where the previous behaviour is written down for when cross-state opens up. **Sir Peter should be told plainly that this is what he traded away**, so it is a choice on the record rather than a surprise later. |
| **✅ Conflict RESOLVED — State Boundary Rule is in force** | 2026-08-26. Sir Peter confirmed and named both rules. **Event Location Rule** — event location when provided, home address as fallback. **State Boundary Rule** — matching is always by the client's home state; no cross-state, even if the event is out of state. The **boundary rule overrides** the other ("each state has its own rules/laws so until we can figure it all out then we will at least get this problem resolved"), so the Event Location Rule is recorded but NOT in force. `StateMatching::requestState()` now returns the client's home state unconditionally and is the single place the rule lives; when cross-state opens up it is the only function that changes. The `event_state` selector is gone from the BR and ER forms — it could not change the outcome, and its hint ("Professionals in this state are the ones who can bid") was untrue for any state but the client's own; both forms now state plainly who will see the request. Posting and direct hiring finally give the same answer for the same pair. |
